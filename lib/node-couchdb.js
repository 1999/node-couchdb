...
export default class NodeCouchDB {
    ...
    /**
    /**
     * Subscribes to the _changes endpoint.  The query params should be set to feed=continuous and timeout=Infinite
     * @param dbName
     * @param query
    ...
    _requestWrapped(opts) {
        ...
        if (opts.changesCallback) {
            return new Promise((resolve, reject) => {
                const url = 'http://localhost:5984/medusa-connector/_changes?feed=continuous&heartbeat=true&since=now';
                ...
                var authorizationBasic = btoa('zoe' + ':' + 'zoe123');
