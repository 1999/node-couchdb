...
export default class NodeCouchDB {
    constructor(opts = {}) {
        ...
        this._timeout = setTimeout(() => controller.abort(), instanceOpts.timeout);

        this._baseUrl = `${instanceOpts.protocol}://${instanceOpts.auth.user}:${instanceOpts.auth.pass}@${instanceOpts.host}:${instanceOpts.port}`;
    ...
    get(dbName, uri, query = {}) {
        const url = new URL(`${this._baseUrl}/${dbName}/${encodeURIComponent(uri)}`); 
    ...
    updateFunction(dbName, designDocument, updateFunctionName, queryString, docId ) {
        ...
        if (method === 'PUT') {
            url = new URL(`${this._baseUrl}/${dbName}/_design/${designDocument}/_update/${updateFunctionName}/${encodeURIComponent(docId)}`);
        } else {
